...
parser.add_argument('--model', type=str, default = 'weights/halfpcd_to_completepcd.pth',  help='optional reload model path')
...
partial_dir = "data/test/partial/"
gt_dir = "data/test/main/"
...
with torch.no_grad():
    for i, model in enumerate(os.listdir(partial_dir)):
        print(model)
        partial = torch.zeros((50, 5000, 3), device='cuda')
        gt = torch.zeros((50, opt.num_points, 3), device='cuda')
        
        for j in range(50):
            pcd = o3d.io.read_point_cloud(os.path.join(partial_dir, model))
            partial[j, :, :] = torch.from_numpy(resample_pcd(np.array(pcd.points), 5000))
            pcd = o3d.io.read_point_cloud(os.path.join(gt_dir, model))
        ...
        vis.scatter(X = output2[idx].data.cpu(),
                    win = 'OUTPUT',
                    opts = dict(title = model, markersize=2))
        print(opt.env + ' val [%d/%d]  emd1: %f emd2: %f expansion_penalty: %f' %(i + 1, len(os.listdir(partial_dir)), emd1, emd2, expansion_penalty.mean().item()))